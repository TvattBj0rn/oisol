...
class Damage:
    def __init__(self, raw_damage: str, damage_multiplier: str | None, damage_rng: str | None):
        ...
        self._damage_multiplier = float(damage_multiplier) if damage_multiplier else 1.0
...
class WikiTemplate:
    ...
    def _generate_armament_value(self, armament_id: int) -> str:
        ...
        if not is_armament_valid:
            return ''
        return (
            f'{main_name if main_name  else self._raw_data.get('name')}{f' ({EMOJIS_FROM_DICT.get(ammo_name)})' if ammo_name  else ''}\n'
            + (f'- Velocity of {velocity_mod}%\n' if velocity_mod  else '')
            + (f'- Reload time of {reload_time}s\n' if reload_time  else '')
            + (f'- Range of {gun_range}m' if gun_range  else '')
            + (f', effective range of {effective_range}m' if effective_range  else '')
            + (f', retaliates up to {retaliation_range}m' if retaliation_range  else '')
            + '\n'
        )
...
class VehicleTemplate(WikiTemplate):
    def __init__(self, data_dict: dict):
        ...
        self._categories_attributes = {
            ...
            'MISC': [
                self._create_formatted_attribute('Class', str(vic_type) if (vic_type := self._raw_data.get('vehicle type'))  else ''),
                self._create_formatted_attribute('Crew', str(crew) if (crew := self._raw_data.get('crew'))  else ''),
                self._create_formatted_attribute('Passengers', str(passengers) if (passengers := self._raw_data.get('passengers'))  else ''),
    ...
    def __init__(self, data_dict: dict):
        ...
        self._categories_attributes = {
            'SURVIVABILITY': [
                ...
                f'\nDecay start after: {convert_time_to_readable_time(float(decay_start))}' if (decay_start := self._raw_data.get('decay start'))  else ''}{
                f'\nDecay duration: {convert_time_to_readable_time(float(decay_duration))}' if (decay_duration := self._raw_data.get('decay duration'))  else ''
                }'),
                self._create_formatted_attribute('Husk & Husk decay', f'{f'{husk_hp} HP' if (husk_hp := self._raw_data.get('husk hp'))  else ''}{
                f'\nDecay start after: {convert_time_to_readable_time(float(husk_decay_start))}' if (husk_decay_start := self._raw_data.get('husk decay start'))  else ''}{
                f'\nDecay duration: {convert_time_to_readable_time(float(husk_decay_duration))}' if (husk_decay_duration := self._raw_data.get('husk decay duration'))  else ''
                }'),
                self._add_armor_attribute(),
            ],
            'STRUCTURE SUPPORT': [
...
class ItemTemplate(WikiTemplate):
    def __init__(self, data_dict: dict):
        ...
        self._categories_attributes = {
            'ITEM': [
                self._create_formatted_attribute('Category', f'{f'{item_category}{f' ({category_emoji})' if (category_emoji := EMOJIS_FROM_DICT.get(item_category))  else ''}'}' if (item_category := self._raw_data.get('category'))  else ''),
                ...
                self._create_formatted_attribute('Amount per crate', f'{crate_amount}' if (crate_amount := self._raw_data.get('crate amount'))  else ''),
            ],
            'ARMAMENT': [
                self._create_formatted_attribute('Firing mode', f'{firing_mode}' if (firing_mode := self._raw_data.get('firing mode'))  else ''),
                ...
                self._create_formatted_attribute(
                    'Range',
                    (f'- Effective range: {range_effective}m' if (range_effective := self._raw_data.get('range effective'))  else '')
                    + (f'\n- Maximum range: {range_max}m' if (range_max := self._raw_data.get('range max'))  else '')
                    + (f'\n- Effective range (semi automatic mode): {range_effective2}m' if (range_effective2 := self._raw_data.get('range effective2'))  else '')
                    + (f'\n- Maximum range (semi automatic mode): {range_max2}m' if (range_max2 := self._raw_data.get('range max2'))  else ''),
                ),
                self._create_formatted_attribute(
                    'Ammunition',
                    (f'Shoots {all_ammo}' if (all_ammo := ', '.join(f'{v}{f' ({EMOJIS_FROM_DICT.get(self._raw_data[k])})'}' for k in ['ammo', 'ammo2', 'ammo3', 'ammo4'] if (v := self._raw_data[k]) )) else '')
                ...
                self._create_formatted_attribute('Damage', f'{Damage(self._raw_data.get('damage'), self._raw_data.get('damage multiplier'), self._raw_data.get('damage rng')).get()} ({EMOJIS_FROM_DICT.get(self._raw_data.get('damage type'))})' if self._raw_data.get('damage')  else ''),
            ],
        }
