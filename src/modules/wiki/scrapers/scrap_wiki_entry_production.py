        ...
        for i, td in enumerate(tr.find_all('td')):
            ...
            match i:
                ...
                case 1:
                    ...
                    wiki_response_dict[list(wiki_response_dict.keys())[i]].append(
                        [((input_row := ' '.join((x, y))), EMOJIS_FROM_DICT.get((item_name := re.sub(remover_pattern, '', input_row)), item_name)) for x, y in zip(split_text[::2], split_text[1::2])]
                    )
            # 2 -> Outputs
                case 2:
                    ...
                    if not len(split_text) % 2 and not split_text[0].strip().isdigit():
                        wiki_response_dict[list(wiki_response_dict.keys())[i]].append(
                            [(output_row := (' '.join((x, y))), re.sub(remover_pattern, '', output_row)) for x, y in zip(split_text[::2], split_text[1::2])]
                        )
                    else:
                        wiki_response_dict[list(wiki_response_dict.keys())[i]].append(
