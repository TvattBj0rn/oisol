...
class ModuleStockpiles(commands.Cog):
    ...
    def refresh_stockpile_interface(bot: Oisol, guild_id: int) -> discord.Embed:
        ...
        for stockpile in guild_stockpiles:
            ...
            grouped_stockpiles[stockpile[0]][f'{stockpile[1]}_{stockpile[4]}'][stockpile[3]] = stockpile[2]
        ...
        # Get group faction
        config = configparser.ConfigParser()
        config.read(bot.home_path / str(guild_id) / DataFilesPath.CONFIG.value)
    ...
    async def stockpile_create(self, interaction: discord.Interaction, code: str, localisation: str, *, name: str) -> None:
        ...
        shard_name = get_current_shard(self.bot.home_path / str(interaction.guild_id) / DataFilesPath.CONFIG.value, code)
    ...
    async def region_autocomplete(self, interaction: discord.Interaction, current: str) -> list[app_commands.Choice]:
        ...
        current_shard = get_current_shard(self.bot.home_path / str(interaction.guild_id) / DataFilesPath.CONFIG.value, code)
