...
class ModuleRegister(commands.Cog):
    ...
    async def register_view(self, interaction: discord.Interaction):
        ...
        register_members = CsvHandlerRegister(self.csv_keys).csv_get_all_data(generate_path(interaction.guild.id, DataFilesPath.REGISTER.value))
        for member in register_members:
            register_embed.add_field(name='', value=f'<@{member[self.csv_keys[0]]}> **|** <t:{member[self.csv_keys[1]]}:R>', inline=False)
    ...
    async def register_add(self, interaction: discord.Interaction, member: discord.Member):
        ...
        CsvHandlerRegister(self.csv_keys).csv_append_data(generate_path(interaction.guild.id, DataFilesPath.REGISTER.value), {self.csv_keys[0]:recruit_id, self.csv_keys[1]: recruit_timer})
    ...
    async def register_clean(self, interaction: discord.Interaction):
        ...
        CsvHandlerRegister(self.csv_keys).csv_rewrite_file(generate_path(interaction.guild.id, DataFilesPath.REGISTER.value), updated_recruit_list)
