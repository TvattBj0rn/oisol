...
from src.modules.wiki.module_wiki import HEALTH_DATA, PRODUCTION_DATA, WIKI_DATA
from src.modules.wiki.wiki_embeds_templates import WikiTemplateFactory
from src.utils import WikiTables

ITEMS_TABLE_ALL_FIELDS = ['version', 'codename', 'aliases', 'name', 'image', 'imageOverride', 'staticImage', 'type', 'faction', 'category', 'uses', 'variants', 'encumbrance', 'encumbrance_worn', 'encumbrance_bonus', 'slot', 'volume', 'damage', 'damage2', 'damage_no_bug', 'damage_rng', 'damage_type', 'AddedBurning', 'BreachingModifier', 'bIgnoreBreachesBunkersThreshold', 'bApplyTankArmourMechanics', 'degrades_armour', 'ArmourDamageModifier', 'TankArmourPenetrationFactor', 'is_mounted', 'damage_multiplier', 'damage_multiplier2', 'explosion_radius_outer', 'explosion_radius_inner', 'fire_rate', 'fire_rate2', 'firing_time', 'firing_mode', 'fuze', 'range_effective', 'range_effective2', 'range_max', 'range_max2', 'HalfAngleMin', 'HalfAngleMax', 'HalfAngleMin2', 'HalfAngleMax2', 'StabilityCostPerShot', 'StabilityCostPerShot2', 'Agility', 'StabilityFloorFromMovement', 'StabilityFloorFromMovement2', 'StabilityGainRate', 'magazine', 'reload', 'ammo', 'ammo2', 'ammo3', 'ammo4', 'intel_range', 'packable', 'crate_amount', 'pallet_amount', 'status', 'GalleryName']
MAPS_TABLE_ALL_FIELDS = ['version', 'codename', 'aliases', 'name', 'image', 'staticImage', 'victory', 'town', 'relic', 'keep', 'status']
...
PRODUCTION_TEST_DATA = [f'{entry['name']}@{entry['table']}' for entry in PRODUCTION_DATA]
...
@pytest.fixture(scope='session')
def item_data():
    return request_table_data(ITEMS_TABLE_ALL_FIELDS, 'itemdata')


@pytest.fixture(scope='session')
def map_data():
    return request_table_data(MAPS_TABLE_ALL_FIELDS, 'maps')


@pytest.mark.parametrize('search_request', HEALTH_TEST_DATA)
def test_health_command(structure_data, vehicle_data, search_request: str):
...
def test_wiki_command(structure_data, vehicle_data, map_data, item_data, search_request: str):
    search_request, health_table = search_request.split('@')

    if health_table == 'vehicles':
        data_dict = vehicle_data[search_request]
    elif health_table == 'structures':
        data_dict = structure_data[search_request]
    elif health_table == 'maps':
        data_dict = map_data[search_request]
    else:
        data_dict = item_data[search_request]

    data_dict['armour type'] = 'None'
    data_dict['image_url'] = ''
    data_dict['damages'] = ''
    data_dict['armor_attributes'] = {'A': '0.95'}

    embed_dict = WikiTemplateFactory(data_dict).get(WikiTables(health_table)).generate_embed_data()

    assert len(embed_dict), f'Health embed is empty for {search_request}'
